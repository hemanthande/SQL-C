...
/*Interface Tables AIF records*/
SELECT TableID = AIF.TABLE_ID
	,InterfaceTableName = AIF.NAME
	--,AIF.DIR_TYPE_C
	,DIR_TYPE_C = DType.TITLE
	,AIF.TABLE_SPECIFICATION_ID
	,AIF.CASE_INSENSITIVE_YN
	,AIF.INCOMING_DEFAULT_VALUE
	,AIF.OUTGOING_DEFAULT_VALUE
	,AIF.EMFI_ENABLED_YN
	,VAL.LINE
	,INT_VALUE
	,EXT_VALUE
FROM dbo.INTERFACE_TABLES AIF
LEFT OUTER JOIN dbo.ZC_DIR_TYPE AS DType ON AIF.DIR_TYPE_C = DType.DIR_TYPE_C
LEFT JOIN INTERFACE_TBL_VALS Val ON AIF.TABLE_ID = Val.TABLE_ID
ORDER BY TableID
	,Val.Line
